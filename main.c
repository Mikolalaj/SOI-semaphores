...
#define MAX_ITEMS 2 // Maximum items a producer can produce or a consumer can consume
...
void *produce(void *arguments)
{
    ...
    while (1) {
        ...
        printf("Producer %d: Created \"%s\" (%d/5)\n",
            index, buffer->item_name, value+1);
...
void *consume(void *arguments)
{
    ...
    while (1) {
        ...
        printf("Consumer %d: Made pierog with \"%s\" (%d/5) (Ciasto %d/5)\n",
            index, buffer_nadzienie->item_name, value_nadzienie, value_ciasto);
